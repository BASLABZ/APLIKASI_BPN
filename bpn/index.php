      ...
      if (isset($_GET['logout'])) {
          session_destroy();
            ...
            { if ($_SESSION['level'] == "bpn")
               { ?>
               <?php // menampilkan kota 
                 $row_kota = mysql_fetch_array(mysql_query("SELECT * from kota where idkota = '".$_SESSION['idkota']."'"));
              ?>
<!DOCTYPE html>
<html>
<head>
  ...
  <title>SISTEM INFORMASI BPN-PPAT <?php echo $row_kota['nama_kota']; ?></title>
 ...
 $('#table').DataTable({"scrollX": true});
    $('#tgl_periode').datepicker();
      $('.select2').select2();
        ...
        // untuk cetak laporan perperiode awal
    $( "#periodeawal" ).datepicker({
        onClose: function() {
            var dateperiodeawal = $('#periodeawal').datepicker('getDate');
             dateperiodeawal.setDate(dateperiodeawal.getDate()+6)
    ...
    // untuk cetak laporan perperiode akhir
    $( "#periodeakhir" ).datepicker({
        onClose: function() {
            var dateperiodeakhir = $('#periodeakhir').datepicker('getDate');
             dateperiodeakhir.setDate(dateperiodeakhir.getDate()+6)
        },beforeShowDay: function(date) {
        var day = date.getDay();
        return [(day != 0 && day != 6)];
    }
