<?php error_reporting(0); include '../../koneksi/koneksi.php';
	session_start();
	$idkota  = $_SESSION['idkota'];
	$kota  = mysql_fetch_array(mysql_query("SELECT * FROM kota where idkota = '".$idkota."'"));
	$idpengguna  = $_POST['idpengguna'];
	$query_show = mysql_query("SELECT * FROM ppat p JOIN pengguna pe ON p.idpengguna = pe.idpengguna where
				    	pe.idkota='".$idkota."' AND p.idpengguna = '".$idpengguna."'");
...
<body onload="window.print()">
		...
		<table>
				...
				<p align="center">
				    ...
				    <?php 
				    	$no = 1;
				    	$query = mysql_query("SELECT * FROM ppat p JOIN pengguna pe ON p.idpengguna = pe.idpengguna where
				    	pe.idkota='".$idkota."' AND p.idpengguna = '".$idpengguna."' ");
